...
{
    [Description("发货通知单--校验、重置")]
    //热启动,不用重启IIS
    [Kingdee.BOS.Util.HotUpdate]
    public class FHTZServicePlugin: AbstractOperationServicePlugIn
    {
        ...
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            ...
            if (Context.CurrentOrganizationInfo.ID == 100026)
            {
                if (FormOperation.Operation.Equals("Audit", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        string upsql = $@"/*dialect*/update T_SAL_DELIVERYNOTICE set F_260_XTLY='' where FID={date["Id"]}";
                        DBUtils.Execute(Context, upsql);
                    }
                }
                else if (FormOperation.Operation.Equals("UncloseBill", StringComparison.OrdinalIgnoreCase))
                {
                    //整单反关闭
                    foreach (var date in e.DataEntitys)
                    {
