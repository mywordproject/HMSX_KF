...
namespace HMSX.SCZZ.SCGL
{
    ...
    [Description("成品订单清除半成品生产订单信息_反审核")]
    public class ZXSCDDBCPQC: AbstractOperationServicePlugIn
    {
                ...
                foreach (DynamicObject entity in e.DataEntitys)
                {
                    ...
                    foreach (DynamicObject entry in entrys)
                    {
                        ...
                        if (srcbi.StartsWith("MO"))
                        {
                            ...
                            string sql = $"/*dialect*/update T_PRD_MOENTRY set F_260_BCPXX=REPLACE(F_260_BCPXX,'{info}','') from T_PRD_MO b where T_PRD_MOENTRY.FID=b.FID and b.FBILLNO='{srcbi}' and T_PRD_MOENTRY.FSEQ={srcseq};";
