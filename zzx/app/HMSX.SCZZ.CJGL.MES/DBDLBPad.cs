...
namespace HMSX.SCZZ.CJGL.MES
{
    ...
    public class DBDLBPad:AbstractMobilePlugin
    {
        ...
        {
            string DBSQL = string.Format(@"/*dialect*/select distinct a.FBILLNO 单据编号,wl.FNUMBER 物料编码,wla.FNAME 物料名称,b.FLOT_TEXT 批号,b.FQTY 调拨数量,
                ...
                inner join (select distinct bm.FWIPSTOCKID from T_SFC_DSPRPTPERM a                       
                        inner join T_SFC_DSPRPTPERMENTRY b on a.FID=b.FID
                        inner join T_ENG_WORKCENTER zx on zx.FID=b.FWCID
                        inner join T_BD_DEPARTMENT bm on zx.FDEPTID=bm.FDEPTID                        
                        where b.FISCHECKED=1 and a.FUSERID={0}
                        union all
