...
{
    [Description("生产汇报--计划跟踪号")]
    //热启动,不用重启IIS
    [Kingdee.BOS.Util.HotUpdate]
    public class SCHBServicePlugin: AbstractOperationServicePlugIn
    {
        readonly string[] reloadKeys = new string[] { "F_260_JHGZHBM", "F_260_XMHH", "FMtoNo" };
        public override void OnPreparePropertys(PreparePropertysEventArgs e)
        {
            base.OnPreparePropertys(e);
            String[] propertys = {  "FMaterialID", "F_260_XMHH", "FHMSXKHCJDYB", "FMtoNo" };
                ...
                if (FormOperation.Operation.Equals("Save", StringComparison.OrdinalIgnoreCase))
                {
                    if (Context.CurrentOrganizationInfo.ID == 100026)
                    {
                        ...
                        foreach (ExtendedDataEntity extended in e.SelectedRows)
                        {
                            ...
                            foreach (var date in dates["PRD_MORPTENTRY"] as DynamicObjectCollection)
                            {
                                    ...
                                    (date["MtoNo"] == null || date["MtoNo"].ToString() == "" || date["MtoNo"].ToString() == " "))
                                {
                                    ...
                                    if (cxs.Count > 0)
                                    {
                                        ...
                                        long id = 0;
                                        string xmhsql = $@"select MIN(FPKID)FPKID FROM PAEZ_t_Cust_Entry100370";
               ...
               if (FormOperation.Operation.Equals("Save", StringComparison.OrdinalIgnoreCase))
                {
                    if (Context.CurrentOrganizationInfo.ID == 100026)
                    {
                        ...
                        {
                            foreach (var entry in date["PRD_MORPTENTRY"] as DynamicObjectCollection)
                            {
                                if (((DynamicObject)entry["MaterialID"])["Number"].ToString().Substring(0, 6) == "260.02")
                                {
                                    if (entry["MtoNo"] == null ||entry["MtoNo"].ToString() == "" || entry["MtoNo"].ToString() == " ")
                                    {
                                        ...
                                        if (jy.Count > 0)
                                        {
                                            ...
                                            string upsql = $@"/*dialect*/ update T_PRD_MORPTENTRY set FMTONO='{str}',F_260_JHGZHBM='{str1}' where FENTRYID={entry["Id"]}";
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            string upsql = $@"/*dialect*/ update T_PRD_MORPTENTRY set F_260_JHGZHBM='{str2}' where FENTRYID={entry["Id"]}";
                                            DBUtils.Execute(Context, upsql);
                                        }
                                        catch
                                        {
